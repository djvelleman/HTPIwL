...
The easiest way to get started with Lean is to use GitHub Codespaces in your web browser.  (Note that when using Codespaces it is recommended that you use a Chromium-based browser such as Google Chrome or Microsoft Edge.  Codespaces may not work in other browsers.)  To open the HTPI Lean Package in Codespaces, click [here](https://codespaces.new/djvelleman/HTPILeanPackage){target="_blank"}.
...
Next, in VS Code, select "Open Folder ..." from the File menu and open the folder containing the HTPI Lean Package that you downloaded.  (If VS Code asks if you want to open the package in a "container", it is probably best to say no.)  Under the heading "Explorer" on the left side of the window, you should see a list of the files in the package.  (If you don't see the list, try clicking on the *Explorer* icon at the top of the left sidebar; it looks like this: ![](Images/Explorer.png).)  Your screen should look similar to the image above in the section about using GitHub Codespaces.  Click on the file "Blank.lean" in the file list.  Then click on the "$\forall$" icon, and in the pop-up menu select "Project Actions ... > Build Project".  Lean should "build" the HTPI Lean Package.  You'll see a number of messages, including some warnings that you can ignore.  Eventually you should see the message "Project built successfully."  Your installation is now complete.
