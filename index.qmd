...
In addition to the file "Blank.lean," the package contains a file for each chapter that contains Lean code, with all Lean definitions and theorems from that chapter.  There are also files containing all the exercises.  In the exercise file for a chapter, all definitions and theorems from that chapter and all earlier chapters are available for use in solving the exercises.

The package also contains a number of other files that are needed to make the package work.  *Do not edit these files*, or you will need to redownload the package.
