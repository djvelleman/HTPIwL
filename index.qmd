...
Click on the "Install Lean using Elan" button, and Lean should be installed.  Then Lean should "build" the HTPI Lean package.  This may take a while, but it only has to be done once.  If you are comfortable giving commands at a command line in a terminal window, you can speed up this process as follows:  In VS Code, select "New Terminal" from the Terminal menu.  At the command line, type `lake exe cache get`.  You will see a number of messages about files being dowloaded.
...
To use the Lean package that accompanies this book in Gitpod, click on this link:

[![](Images/open-in-gitpod.png)](https://gitpod.io/#https://github.com/djvelleman/HTPILeanPackage)

You will be prompted to create a Gitpod account if you don't already have one; a free account gives you 50 hours of use per month.  Then you will be asked to create a "workspace".  Click on "Continue".  It will take some time to create your workspace and initialize Lean, but this only needs to be done once.  You will see messages about "cloning", "building", "downloading", and "decompressing".  When that process is complete, you should see the HTPI Lean package, as described in the previous section.  Later, you can return to the workspace you just created from your [Gitpod user page](https://gitpod.io/workspaces).
