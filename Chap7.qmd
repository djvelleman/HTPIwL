...
This tells Lean that if we type `a ≡ b (MOD m)`, then Lean should interpret it as `congr_mod m a b`.  (To enter the symbol `≡`, type `\==`.)
...
Thus, `cc m a` is the congruence class modulo `m` of `a`; it corresponds to the notation $[a]_m$ in *HTPI*.  Once again, it will be convenient to teach Lean to use more familiar notation for congruence classes, so we give the command:

```lean
notation:max "["a"]_"m:max => cc m a
```

Now if we type `[a]_m`, then Lean will interpret it as `cc m a`.  Thus, from now on, `[a]_m` will be our notation in Lean for the congruence class modulo `m` of `a`, corresponding to the *HTPI* notation $[a]_m$.  (Don't worry about the two occurrences of `:max` in the `notation` command above.  They are there to help Lean parse this new notation when it occurs together with other notation.)
