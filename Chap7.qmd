...
To establish the main properties of `gcd a b` we'll need several lemmas.  We prove some of them and leave others as exercises.  In the first lemma below, you might think that the `rfl` tactic would prove the goal, because our definition of `gcd` says that if `a` is a natural number, then `gcd a 0` means `a`.  However, for technical reasons that we won't go into here, our definition of `gcd` is complicated enough that Lean doesn't automatically recognize `gcd a 0` as meaning the same thing as `a`.  Fortunately, there is a variant of the `rewrite` tactic that solves the problem.  The tactic `rewrite [gcd]` causes Lean to rewrite `gcd a 0` as `a`, and then the `rfl` tactic can complete the proof. 
...
Our main theorem about these functions is that they give the coefficients needed to write `gcd a b` as a linear combination of `a` and `b`.  As usual, stating a few lemmas first helps with the proof.  We leave the proofs of three of them as exercises for you (hint: imitate the proofs about `gcd` above).
...
lemma gcd_c2_base (a : Nat) : gcd_c2 a 0 = 0 := sorry
...
::: {.ind}
```
Int.mul_ediv_add_emod : ∀ (a b : ℤ), b * (a / b) + a % b = a
...
Int.natAbs_natCast : ∀ (n : ℕ), Int.natAbs ↑n = n
```
:::
