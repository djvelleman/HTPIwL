...
Now if we type `[a]_m`, then Lean will interpret it as `cc m a`.  Thus, from now on, `[a]_m` will be our notation in Lean for the congruence class modulo `m` of `a`; it corresponds to the *HTPI* notation $[a]_m$.  (Don't worry about the two occurrences of `:max` in the `notation` command above.  They are there to help Lean parse this new notation when it occurs together with other notation.)
