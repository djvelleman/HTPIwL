...
For clarity, we have written the coercions explicitly in the statement of the inductive hypothesis.  In case 2, we have introduced the variables `r`, `q`, `s`, and `t` to simplify the notation.  After the step `rewrite [←h3, ←h4]`, the goal contains the expression `↑(b * q + r)`.  You can use the `#check` command to see why `Nat.cast_add` and `Nat.cast_mul` convert this expression to first `↑(b * q) + ↑r` and then `↑b * ↑q + ↑r`.
...
To study congruence modulo `m` in Lean, we will declare `m` to have type `Nat`, which allows for the possibility that `m = 0`, but we will soon focus on the case `m > 0`.  For `a` and `b` of type `Int`, we define `congr_mod m a b` to mean that `a` is congruent to `b` modulo `m`.  Notice that to define this relation in Lean, we must coerce `m` to be an integer so that we can use it in the divisibility relation on the integers.  We then prove that `congr_mod m` is reflexive, symmetric, and transitive.  As usual, we leave some details as exercises.
...
We now have all the background we need to prove Theorem 7.3.1 in Lean.  We begin with a few lemmas, some of which require the hypothesis `[NeZero m]`.  The proof of the theorem closely follows the proof in *HTPI*.
...
Which congruence classes are invertible?  The answer is given by Theorem 7.3.7 in *HTPI*, which says that if $a$ is a positive integer, then a $[a]_m$ is invertible if and only if $m$ and $a$ are relatively prime.  The proof uses an exercise from the last section.  Here is the Lean version of the proof.
...
Since `{0, 1, ..., m - 1}` is a complete residue system modulo `m`, `phi m` can be described as the number of natural numbers `a < m` such that `cc m a` is invertible.  (Note that `cc m a` is really `cc m ↑a`, since `a` must be coerced to `Int` to be used in our congruence class notation, but we will usually let Lean figure out the coercions for us.)  According to `Theorem_7_3_7`, `cc m a` is invertible if and only if `m` and `a` are relatively prime, so `phi m` is also equal to the number of natural numbers `a < m` that are relatively prime to `m`.  We define a function `num_rp_below m k` that counts the number of natural numbers less than `k` that are relatively prime to `m`, and then use it to define `phi`.
...
lemma num_rp_below_step_not_rp {m j : Nat} (h : ¬rel_prime m j) :
    num_rp_below m (j + 1) = num_rp_below m j := sorry
...
With this preparation, we can now state the theorem:
...
To express the `F`-product in Lean, we imitate our approach to summations, as described in Chapter 6.  We begin by defining `prod_seq m k f` to be the product of a sequence of `m` consecutive values of the function `f`, starting with `f k`:
...
lemma prod_one {A : Type} [MulOneClass A]
    (k : Nat) (f : Nat → A) : prod_seq 1 k f = f k := sorry
```

(The type class `MulOneClass A` indicates that `A` must be a type for which `*` and `1` make sense.)  Using this notation, the expression `prod_seq m 0 (F m)` denotes the `F`-product, which, as we saw earlier, is equal to the product of the invertible congruence classes.
...
Once again, we will be interested in the partial application `G m a`, which is a function from `Nat` to `Nat`.  Consider the following product, which we will call the *`FG`-product*:

::: {.ind}
`(F m (G m a 0)) * (F m (G m a 1)) * ... * (F m (G m a (m - 1)))`.
:::

We can express this in Lean as `prod_seq m 0 ((F m) ∘ (G m a))`.

If `i` is not relatively prime to `m`, then `F m (G m a i) = F m i = cc m 1`, so as before these terms contribute nothing to the `FG`-product.  To understand the contribution of the other terms, we will use two lemmas.

```lean
lemma G_rel_prime {m a i : Nat}
    (h1 : rel_prime m a) (h2 : rel_prime m i) :
    rel_prime m (G m a i) := sorry
...
According to the lemma `G_rel_prime`, if `i` is relatively prime to `m` then `G m a i` is also relatively prime to `m`, so `F m (G m a i) = cc m (G m a i)`.  Filling in the definition of `G m a i` and applying the lemma `cc_eq_mod`, this means

::: {.ind}
`F m (G m a i) = cc m ((a * i) % m) = cc m (a * i) = (cc m a) * (cc m i)`.
:::

Thus, `F m (G m a i) = (cc m a) * (F m i)`.  This means that, in the `FG`-product, each factor contributed by a value of `i` that is relatively prime to `m` is `cc m a` times the corresponding factor in the `F`-product.  Since the number of such factors is `phi m`, it follows that the `FG`-product is `(cc m a) ^ (phi m)` times the `F`-product.
...
To prove these claims, we first define what it means for a function to permute the natural numbers less than `m`.
...
The proofs of our next two lemmas are somewhat long.  We state them now, but put off discussion of their proofs until the end of the section
...
lemma perm_prod {R : Type} [CommRing R] (f : Nat → R) :
    ∀ (m : Nat), ∀ (g : Nat → Nat), perm_below m g →
      prod_seq m 0 f = prod_seq m 0 (f ∘ g)
```

The type class `CommRing R` indicates that the type `R` must have operations of addition and multiplication that satisfy all of the algebraic laws in Theorem 7.3.6 of *HTPI*.  Lean knows that `ZMod m` satisfies all of those laws, so it knows that the lemma `perm_prod` can be applied with `R = ZMod m`.

There is just one more fact we need before we can prove `Theorem_7_4_2`: a product of invertible congruence classes is invertible, and therefore the `F`-product is invertible.
