...
Looking at `h3` and `h6`, we see that the right value to plug in for `x` in the goal is `a`, so our next tactic is `apply Exists.intro a`.
...
Finally, we can complete the proof by combining `h3` and `h6`.
...
(h2 : ¬∃ (x : U), x ∈ A ∩ B) : A ⊆ C := by
  ...
  have h5 : a ∉ B := by
    contradict h2 with h6
    apply Exists.intro a
    show a ∈ A ∩ B from And.intro h3 h6
    done
  disj_syll h4, h5  --Now h4 : a ∈ C
  show a ∈ C from h4
```
