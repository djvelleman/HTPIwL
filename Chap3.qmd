...
The second strategy is justified by the contrapositive law.  In Lean, you can use the `contrapos` tactic to rewrite the goal as `¬Q → ¬P`; then use the tactic `assume h : ¬Q` to get this result:
...
The goal is `a ∈ C`, and the only given that even mentions `C` is `h3 : a ∈ B → a ∈ C`.  If only we could prove `a ∈ B`, then we could apply `h3`, using modus ponens, to reach our goal.  The tactic `apply h3` asks Lean to work backwards, seeing if it can find a way to apply `h3` to reach the goal. You might think of this tactic as asking Lean the question: If we want `h3 _` to be a proof of `a ∈ C`, what do we have to put in the blank?  Lean is able to figure out that the answer is: a proof of `a ∈ B`.  So it sets the goal to be `a ∈ B`, since a proof of that goal, when combined with `h3`, would prove the theorem:
