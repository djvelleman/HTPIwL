...
Lean responds with `exact one_mul n`, and `#check one_mul` yields `one_mul : ∀ (a : ?m.73050), 1 * a = a`.  So `one_mul n` should justify the last equation.  Here's the complete calculational proof, where we have shortened the second step a bit by doing both `rewrite`s in one step:
