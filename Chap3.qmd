...
We can now make inferences from `h1` and `h2`.  To see how to use the inferred statements, we write out their definitions, and since one of them is a negative statement, we reexpress it as an equivalent positive statement.
...
Recall that `∃! (x : U), P x` means that there is exactly one `x` of type `U` such that `P x` is true.  Usually a goal of this form is proven by using the following strategy.  This is a slight rephrasing of the strategy presented in *HTPI*.

#### To prove a goal of the form `∃! (x : U), P x`:

::: {.ind}
Prove `∃ (x : U), P x` and `∀ (x1 x2 : U), P x1 → P x2 → x1 = x2`.  The first of these goals shows that there exists an `x` such that `P x` is true, and the second shows that it is unique.  The two parts of the proof are therefore sometimes labeled *existence* and *uniqueness*.
:::
...
The next step in *HTPI* was apply `h1` to `D`, and `h2` to `C`.  We do the same thing in Lean.
