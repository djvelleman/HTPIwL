...
| `push Not` | [Section 8.1](Chap8.html#equinumerous-sets) |
...
If the goal is a negative statement, then the tactic `push Not` will apply multiple laws about negations to "push" the negation as far as possible into the statement.
...
We have mostly used these tactics to reexpress negative statements as more useful positive statements.  The tactic `push Not` can be used for this purpose.
...
If your goal is a conditional statement, then the tactics `contrapose` and `contrapose!` will replace the goal with its contrapositive (`contrapose!` also uses `push Not` to try to simplify the negated statements that arise when forming a contrapositive).  You may also find the theorem `not_imp_not` useful:
