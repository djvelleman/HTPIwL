...
theorem Example_6_4_1 : ∀ m > 0, ∀ (n : Nat),
  ...
  · -- Case 2. h2: ¬n < m
    ...
    have h4 : n - m < n := Nat.sub_lt_of_pos_le m n h1 h3
    have h5 : ∃ (q r : Nat), n - m = q * m + r ∧ r < m := ih (n - m) h4
    obtain (q' : Nat)
      (h6 : ∃ (r : Nat), n - m = q' * m + r ∧ r < m) from h5
    obtain (r' : Nat) (h7 : n - m = q' * m + r' ∧ r' < m) from h6
    apply Exists.intro (q' + 1)
    apply Exists.intro r'     --Goal: n = (q' + 1) * m + r' ∧ r' < m
    apply And.intro _ h7.right
    show n = (q' + 1) * m + r' from
      calc n
          = n - m + m := (Nat.sub_add_cancel h3).symm
...
theorem Exercise_6_4_7a : ∀ (n : Nat),
  ...
  · -- Induction Step
    fix n : Nat
    assume ih : (Sum i from 0 to n, Fib i) + 1 = Fib (n + 2)
    show (Sum i from 0 to n + 1, Fib i) + 1 = Fib (n + 1 + 2) from
      calc (Sum i from 0 to n + 1, Fib i) + 1
          = (Sum i from 0 to n, Fib i) + Fib (n + 1) + 1 :=
            by rw [sum_from_zero_step]
        _ = (Sum i from 0 to n, Fib i) + 1 + Fib (n + 1) := by ring
        _ = Fib (n + 2) + Fib (n + 1) := by rw [ih]
        _ = Fib (n + 1) + Fib (n + 1 + 1) := by ring
        _ = Fib (n + 1 + 2) := by rfl
    done
  done
```
...
And we will also use a lemma, whose proof we leave as an exercise for you:
...
```
Hint for \#6:  Let `m` be an arbitrary natural number, and then use induction on `k`.  For the induction step, you must prove `Fib m ∣ Fib (m * (k + 1))`.  If `m = 0 ∨ k = 0`, then this is easy.  If not, then show that you can rewrite `m * (k + 1)` as `(m * k - 1) + m + 1`, and then apply Exercise_6_4_8a.
:::
...
9\. In this exercise you will prove that the numbers `q` and `r` in `Example_6_4_1` are unique.  It is helpful to prove a lemma first.
```lean
