...
So `sum_less 3 f` adds up all values of `f i`, for `i < 3`.  More generally, `sum_less m f` gives the sum of all values `f i` for `i < m`.  (The implicit arguments say that the type of `f i` can be any type for which `0` and `+` make sense.)  The notation `Sum i from k to n, f i` is now defined to be a shorthand for `sum_less (n + 1 - k) (fun (j : Nat) => f (k + j))`.  We'll leave it to you to puzzle out why that gives the desired result.
