...
Before ending this section, we'll return to a topic left unexplained before.  We can now describe how `Sum i from k to n, f i` is defined.  The key is a function `sum_val_from`, which is defined by recursion:

```lean
def sum_val_from {A : Type} [AddZeroClass A]
    (m k : Nat) (f : Nat → A) : A :=
  match m with
    | 0 => 0
...
To get an idea of what this definition means, let's try evaluating `sum_val_from 3 k f`:

::: {.ind}
```
sum_val_from 3 k f = sum_val_from 2 k f + f (k + 2)
             = sum_val_from 1 k f + f (k + 1) + f (k + 2)
             = sum_val_from 0 k f + f (k + 0) + f (k + 1) + f (k + 2)
             = 0 + f (k + 0) + f (k + 1) + f (k + 2)
             = f k + f (k + 1) + f (k + 2)
```
:::

So `sum_val_from 3 k f` adds up three consecutive values of `f`, starting with `f k`.  More generally, `sum_val_from n k f` adds up `n` consecutive values of `f`, starting with `f k`.  (The implicit arguments say that the type of the values of `f` can be any type for which `0` and `+` make sense.)  The notation `Sum i from k to n, f i` is now defined to be a shorthand for `sum_val_from (n + 1 - k) k f`.  We'll leave it to you to puzzle out why that gives the desired result.
...
Imitating the strategy of the proof in *HTPI*, we let `m` be an arbitrary natural number, assume `m > 0`, and then prove the statement `∀ (n : Nat), ∃ (q r : Nat), n = q * m + r ∧ r < m` by strong induction.  That means that after introducing an arbitrary natural number `n`, we assume the inductive hypothesis, which says `∀ n_1 < n, ∃ (q r : Nat), n_1 = q * m + r ∧ r < m`.
...
By the way, although we are unlikely to want to use the notation `n / 0` or `n % 0`, Lean uses the definitions `n / 0 = 0` and `n % 0 = n`.  As a result, the equation `n * (m / n) + m % n = m` is true even if `n = 0`.  That's why the theorem `Nat.div_add_mod` doesn't include a requirement that `n > 0`.
...
In addition to the last lemma, we'll use the theorems `add_lt_add_left` and `add_lt_add_right`; these are similar to the theorem `add_le_add_left`, which we used in `Example_6_3_4`.  We have almost everything that we need:
...
There is one step that we haven't justified.  The following theorem will help us fill in this gap:
...
To apply the theorem, we'll need to know `3 ≤ 4`, which can be proven by `linarith`.  Here's the complete proof:
...
```
Hint for \#7:  Let `m` be an arbitrary natural number, and then use induction on `k`.  For the induction step, you must prove `Fib m ∣ Fib (m * (k + 1))`.  If `m = 0 ∨ k = 0`, then this is easy.  If not, then show that you can rewrite `m * (k + 1)` as `(m * k - 1) + m + 1`, and then apply `Exercise_6_4_8a`.
:::
