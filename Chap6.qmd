...
How do we come up with the number `k` in the previous paragraph?  Since similar reasoning will come up in some later proofs, it will be convenient to state the existence of the number `k` as lemma.  To prove it, we'll use a theorem from Lean's library:
...
<!--
The proof in *HTPI* uses the cases `n < m` and `m ≤ n`.  If `n < m`, then the proof is easy: the numbers `q = 0` and `r​ = n` clearly have the required properties.  In the case `m ≤ n`, the proof in *HTPI* proceeds by applying the inductive hypothesis to the number `k = n - m`.  Since `m > 0`, we have `k < n`, so the inductive hypothesis can be applied.  Notice that if `m > 1`, then `n - m` is not the number immediately preceding `n`; that's why this proof uses strong induction rather than ordinary induction.

Since `n` and `m` have type `Nat`, the subtraction in the formula `n - m` is natural number subtraction, which, as we've seen, can be difficult to work with.  But in this proof we use this formula only in the case when `m ≤ n`, and in that case the subtraction is unproblematic.  Indeed, there are theorems in Lean that tell us that if `m ≤ n`, then `n - m` has the properties we expect.  We will use two such theorems:

::: {.ind}
```
@Nat.sub_lt_of_pos_le : ∀ (a b : ℕ), 0 < a → a ≤ b → b - a < b
...
With that preparation, we are ready to complete the proof.
...
Notice that in the tactic state for the first case, Lean has replaced `n` with `0` in both the inductive hypothesis and the goal.  Similarly, in the second case `n` gets replaced with `1`, and in the third it is replaced with `k + 2`.  The first two cases can be proven by the `norm_num` tactic.  For the third case, we'll use the theorems `add_lt_add_left` and `add_lt_add_right`; these are similar to the theorem `add_le_add_left`, which we used in `Example_6_3_4`.  We'll also need to use the following theorem:

<!--
`n = 0` and `n = 1` separately, since these values are treated separately in the definition of `Fib n`.  Therefore, after introducing the inductive hypothesis, we split the proof into cases depending on whether or not `n < 2`.  In the first case, in which `n < 2`, the only possible values of `n` are `0` and `1`.  In this situation, the tactic `interval_cases n` can be used to split the proof into separate cases for each possible value of `n`.  As you can see in the tactic state below, the `interval_cases n` tactic splits the first case of our proof into two subcases, filling in `0` for `n` in the first subcase and `1` for `n` in the second.
...
Both subcases of case 1 can be proven by the `norm_num` tactic.  For case 2, in which `¬n < 2`, we'll need to use the lemma `exists_eq_add_of_not_lt`, proven above, to write `n` as `k + 2`, for some natural number `k`.  That will allow us to use the third clause in the definition of `Fib n`.  We'll also use the theorems `add_lt_add_left` and `add_lt_add_right`; these are similar to the theorem `add_le_add_left`, which we used in `Example_6_3_4`.  Finally, we'll need to use the following theorem:
-->
...
<!--
Notice that the formula for `Fib (k + 2)` involves the *two* previous values of `Fib`, not just the immediately preceding value.  That is the sense in which the recursion is *strong*.  Not surprisingly, theorems about the Fibonacci numbers are often proven by induction---either ordinary or strong.  We'll illustrate this with a proof that `∀ (n : Nat), Fib n < 2 ^ n`.  This time we'll need to treat the cases `n = 0` and `n = 1` separately, since these values are treated separately in the definition of `Fib n`.  And we'll need to know that if `n` doesn't fall into either of those cases, then it falls into the third case: `n = k + 2` for some natural number `k`.  Let's prove this as a lemma.

We begin by showing that if we have `h1 : n ≠ 0` and `h2 : n ≠ 1`, then we can deduce `n ≥ 2`.  We'll need two theorems from Lean's library:
...
Using the first of these theorems, `Nat.pos_of_ne_zero h1` is a proof of `0 < n`.  But for natural numbers `a` and `b`, Lean treats `a < b` as meaning the same thing as `a + 1 ≤ b`, so this is also a proof of `1 ≤ n`.  Applying the theorem `lt_of_le_of_ne'` to this conclusion and `h2` proves `1 < n`, which is definitionally equal to `2 ≤ n`:
...
Once we know that `n ≥ 2`, we can conclude that `n = k + 2`, where `k = n - 2`:
...
In addition to the last lemma, we'll use the theorems `add_lt_add_left` and `add_lt_add_right`; these are similar to the theorem `add_le_add_left`, which we used in `Example_6_3_4`.  And we'll also use the following theorem:
...
Now that we have everything we need, we can present the proof:
...
The proof uses some definitions and theorems from the exercises of Section 6.1:
...
theorem Exercise_6_1_16a1 :
    ∀ (n : Nat), nat_even n ∨ nat_odd n

theorem Exercise_6_1_16a2 :
    ∀ (n : Nat), ¬(nat_even n ∧ nat_odd n)
```

We will also use a lemma, whose proof we leave as an exercise for you:
...
```
Hint for \#7:  Let `m` be an arbitrary natural number, and then use induction on `k`.  For the induction step, you must prove `Fib m ∣ Fib (m * (k + 1))`.  If `m = 0 ∨ k = 0`, then this is easy.  If not, then use `Nat.exists_eq_add_of_le'` to obtain a natural number `j` such that `m * k = j + 1`, and therefore `m * (k + 1) = j + m + 1`, and then apply `Exercise_6_4_8a`.
:::
