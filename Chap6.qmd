...
theorem Example_6_4_1 : ∀ m > 0, ∀ (n : Nat),
  ...
  by_strong_induc
  fix n : Nat
  assume ih : ∀ k < n, ∃ (q r : Nat), k = q * m + r ∧ r < m
  **done::
```
...
Notice that the formula for `Fib (n + 2)` involves the *two* previous values of `Fib`, not just the immediately preceding value.  That is the sense in which the recursion is *strong*.  Not surprisingly, theorems about the Fibonacci numbers are often proven by induction---either ordinary or strong.  We'll illustrate this with a proof that `∀ (n : Nat), Fib n < 2 ^ n`.  This time we'll need to treat the cases `n = 0` and `n = 1` separately, since these values are treated separately in the definition of `Fib n`.  We'll also use the theorems `add_lt_add_left` and `add_lt_add_right`; these are similar to the theorem `add_le_add_left`, which we used in `Example_6_3_4`.

```lean
??example:: : ∀ (n : Nat), Fib n < 2 ^ n := by
  by_strong_induc
  fix n : Nat
  assume ih : ∀ (k : Nat), k < n → Fib k < 2 ^ k
  by_cases h1 : n = 0
  · -- Case 1. h1: n = 0
...
There are two steps that require further justification.  The statement `2 ≤ n` should follow from `h1 : n ≠ 0` and `h2 : n ≠ 1`.  To prove it, we'll use two theorems:
...
To eliminate the second `sorry` we'll use the theorem:
...
```lean
theorem well_ord_princ (S : Set Nat) (h1 : ∃ (x : Nat), x ∈ S) :
    ∃ (x : Nat), x ∈ S ∧ ∀ (y : Nat), y ∈ S → x ≤ y := by
  contradict h1 with h2
    --h2: ¬∃ (x : Nat), x ∈ S ∧ ∀ (y : Nat), y ∈ S → x ≤ y
  quant_neg                   --Goal: ∀ (x : Nat), ¬x ∈ S
  by_strong_induc
  fix n : Nat
  assume ih : ∀ k < n, k ∉ S  --Goal: ¬n ∈ S
  contradict h2 with h3       --h3: n ∈ S
    --Goal: ∃ (x : Nat), x ∈ S ∧ ∀ (y : Nat), y ∈ S → x ≤ y
  apply Exists.intro n
  apply And.intro h3          --Goal: ∀ (y : Nat), y ∈ S → n ≤ y
  fix y : Nat
  assume h4 : y ∈ S
  have h5 : y < n → ¬y ∈ S := ih y
  contrapos at h5             --h5: y ∈ S → ¬y < n
  have h6 : ¬y < n := h5 h4
  linarith
  done
```
...
theorem sqrt_2_irrat :
  ...
  have h3 : ∃ (x : ℕ), x ∈ S ∧ ∀ (y : ℕ), y ∈ S → x ≤ y :=
    well_ord_princ S h2
  obtain (q : Nat) (h4 : q ∈ S ∧ ∀ (y : ℕ), y ∈ S → q ≤ y) from h3
  have h4l : q ∈ S := h4.left
  define at h4l     --h4l: ∃ (p : ℕ), p * p = 2 * (q * q) ∧ q ≠ 0
