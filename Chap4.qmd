...
Finally, the third statement in Theorem 4.4.6 says that if `R` is a total order, then an `R`-minimal element of a set `B` must be the `R`-smallest element of `B`.  The beginning of the proof is straightforward:
