...
5\excl{}. Here is an incorrect theorem with an incorrect proof.
...
6\excl{}. Is the following theorem correct?  Try to prove it in Lean.  If you can't prove it, see if you can find a counterexample.
...
If `R` has type `BinRel A`, then `partial_order R` is the proposition saying that `R` is reflexive, transitive, and antisymmetric.  We say in this case that `R` is a *partial order on `A`*---or just that `R` is a *partial order*, if `A` is clear from context.  Similarly, `total_order R` says that `R` is a *total order on `A`*.
...
theorem Theorem_4_4_6_2 {A : Type} (R : BinRel A) (B : Set A) (b : A)
  ...
  -- Proof that b is the only minimal element
  fix c : A
  assume h3 : minimalElt R c B
  define at h3
  have h4 : ¬∃ (x : A), x ∈ B ∧ R x c ∧ x ≠ c := h3.right
  quant_neg at h4
  have h5 : ¬(b ∈ B ∧ R b c ∧ b ≠ c) := h4 b
  demorgan at h5
  disj_syll h5 h2.left  --h5 : ¬(R b c ∧ b ≠ c)
  demorgan at h5
  have h6 : R b c := h2.right c h3.left
  disj_syll h5 h6       --h5 : b = c
  show b = c from h5 
```
...
::: {.numex arguments="10"}
```lean
theorem Exercise_4_4_24 {A : Type} (R : BinRel A) :
    smallestElt (sub (A × A)) ((extension R) ∪ (inv (extension R)))
    { T : Set (A × A) |
        extension R ⊆ T ∧ symmetric (RelFromExt T) } := sorry
```
:::